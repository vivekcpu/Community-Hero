...
const reportsSlice = createSlice({
  ...
  reducers: {
    ...
    updateReportCommentsCount: (state, action) => {
      const { reportId, commentsCount } = action.payload;
      const index = state.items.findIndex((item) => item._id === reportId);
      if (index !== -1) {
        state.items[index].commentsCount = commentsCount;
      }
    }
  },
...
export const { resetReports, updateReportCommentsCount } = reportsSlice.actions;
