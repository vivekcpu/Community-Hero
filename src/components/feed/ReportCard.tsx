...
import { upvoteReport, editReport, deleteReport, updateReportCommentsCount } from "../../store/slices/reportsSlice.js";
...
export default function ReportCard({ report, onAuthorClick, onReportUpdated }: ReportCardProps) {
  ...
  // Synchronize state with incoming prop changes
  useEffect(() => {
    setUpvotes(report.upvotes || []);
  }, [report.upvotes]);

  useEffect(() => {
    setCommentsCount(report.commentsCount || 0);
  ...
  const isUpvoted = currentUser ? upvotes.includes(currentUser._id.toString()) : false;
  ...
  const handleUpvote = async (e: React.MouseEvent) => {
    ...
    const nextUpvotes = upvotes.includes(userIdStr)
      ? upvotes.filter((id) => id !== userIdStr)
      : [...upvotes, userIdStr];
    
    setUpvotes(nextUpvotes);

    try {
      const res = await dispatch(upvoteReport(report._id)).unwrap();
      if (res && res.upvotes) {
        ...
        {showComments && (
          <CommentSection
            ...
            onCommentAdded={(newCount) => {
              ...
              dispatch(updateReportCommentsCount({ reportId: report._id, commentsCount: newCount }));
              if (onReportUpdated) {
                onReportUpdated({
                  ...report,
