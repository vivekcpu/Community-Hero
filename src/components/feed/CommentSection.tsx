        ...
        <div className="space-y-4 max-h-[350px] overflow-y-auto pr-1">
          {parentComments.map((comment) => {
            ...
            if (currentUser && commentAuthor) {
              const authorId = typeof commentAuthor === "string" ? commentAuthor : commentAuthor._id;
              if (authorId && authorId.toString() === currentUser._id.toString()) {
                commentAuthor = {
                  ...commentAuthor,
                  _id: currentUser._id,
              ...
              <div key={comment._id} className="space-y-2">
                ...
                {replies.length > 0 && (
                  <div className="pl-6 space-y-3 border-l-2 border-slate-100/80 ml-3.5 mt-1">
                    {replies.map((reply) => {
                      let replyAuthor = reply.author;
                      if (currentUser && replyAuthor) {
                        const rAuthorId = typeof replyAuthor === "string" ? replyAuthor : replyAuthor._id;
                        if (rAuthorId && rAuthorId.toString() === currentUser._id.toString()) {
                          replyAuthor = {
                            ...replyAuthor,
                            _id: currentUser._id,
                            name: currentUser.name,
