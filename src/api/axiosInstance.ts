...
// Request interceptor to inject Authorization header if token exists in sessionStorage
...
axiosInstance.interceptors.response.use(
  ...
  (error) => {
    if (error.response && error.response.status === 401) {
      console.warn("Unauthorized API access - removing active session storage tokens.");
      sessionStorage.removeItem("user");
      sessionStorage.removeItem("token");
      sessionStorage.removeItem("token_active");
